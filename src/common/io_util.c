...
int ioutils_write_file(const char *path, const char *buf, long bufsz)
{
    int ret = 0;
    ...
    if (1 != fwrite(buf, (size_t) bufsz, 1, f))
        ret = -2;
...
int ioutils_read_file(const char *path, char **_buf, long *_bufsz, long maxalloc)
{
    FILE *f = fopen(path, "r");
    if (!f)
        return -1;
